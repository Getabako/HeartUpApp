        ...
        childItem.innerHTML = `
            ...
            <div class="am-child-actions">
                ${hasAssessment ? `
                    ...
                    <button class="am-btn" style="background:#7b1fa2; color:white;" onclick="amStartPlanRenewal('${escapedName}', '${latestFN}')">計画書更新</button>
                ` : `
...
window.amShowChildSupportPlans = async function(childName, latestAssessmentFileName) {
    ...
    if (latestAssessmentFileName) {
        ...
        html += `<div style="margin-bottom:15px; display:flex; gap:10px; flex-wrap:wrap;">
            <button class="am-btn am-btn-secondary" onclick="amGenerateSupportPlan('${esc}')">＋ 新しい支援計画を作成</button>
            <button class="am-btn" style="background:#7b1fa2; color:white;" onclick="amStartPlanRenewal('${escName}', '${esc}')">計画書更新（振り返り→意向→自動作成）</button>
        </div>`;
    }
...
// HTMLエスケープ（スタッフ入力の意向を計画書HTMLへ安全に埋め込む用）
...
// HTML→テキスト変換（振り返りレポートをプロンプトに渡す用）
function amStripHtml(html) {
    const div = document.createElement('div');
    div.innerHTML = html || '';
    return (div.textContent || '').replace(/\s+/g, ' ').trim().slice(0, 3000);
}

// === 支援計画を生成（入口：意向の聞き取り入力モーダルを表示） ===
window.amGenerateSupportPlan = async function(fileName) {
    ...
    amShowIntentModal(assessment);
};

// 意向入力モーダル（本人・家族の生活に対する意向を自由記述で聞き取り）
function amShowIntentModal(assessment) {
    let existing = document.getElementById('amIntentModal');
    if (existing) existing.remove();

    const modal = document.createElement('div');
    modal.id = 'amIntentModal';
    modal.style.cssText = 'display:flex; position:fixed; top:0; left:0; width:100%; height:100%; background:rgba(0,0,0,0.5); justify-content:center; align-items:center; z-index:1100;';
    modal.innerHTML = `
        <div style="background:white; border-radius:16px; padding:1.5rem; max-width:560px; width:92%; max-height:85vh; overflow-y:auto; position:relative;">
            <span id="amIntentModalClose" style="position:absolute; top:0.5rem; right:1rem; font-size:1.5rem; cursor:pointer; color:#666;">&times;</span>
            <h3 style="margin:0 0 0.5rem; color:#2e7d32;">${amEscapeHtml(assessment.data.childName)}さんの支援計画を作成</h3>
            <p style="font-size:0.9rem; color:#555; line-height:1.7; margin-bottom:1rem;">
                聞き取った「利用児及び家族の生活に対する意向」を入力してください。<br>
                入力した内容は<strong>そのまま計画書の意向欄に記載</strong>されます。（空欄の場合はアセスメントからAIが推定します）
            </p>
            <label style="display:block; font-weight:bold; margin-bottom:0.3rem;">本人の生活に対する意向</label>
            <textarea id="amIntentSelf" rows="3" style="width:100%; padding:0.5rem; border:1px solid #ddd; border-radius:8px; font-size:0.95rem; box-sizing:border-box;" placeholder="例：サッカーがうまくなりたい。友だちともっと遊びたい。"></textarea>
            <label style="display:block; font-weight:bold; margin:0.8rem 0 0.3rem;">家族の生活に対する意向</label>
            <textarea id="amIntentFamily" rows="3" style="width:100%; padding:0.5rem; border:1px solid #ddd; border-radius:8px; font-size:0.95rem; box-sizing:border-box;" placeholder="例：気持ちの切り替えができるようになってほしい。学校でも落ち着いて過ごせるようになってほしい。"></textarea>
            <div style="display:flex; gap:0.5rem; justify-content:flex-end; margin-top:1.2rem;">
                <button id="amIntentModalCancel" style="padding:0.6rem 1rem; border:1px solid #ddd; border-radius:8px; background:white; cursor:pointer;">キャンセル</button>
                <button id="amIntentModalSubmit" style="padding:0.6rem 1.2rem; border:none; border-radius:8px; background:#2e7d32; color:white; cursor:pointer; font-weight:bold;">支援計画を作成する</button>
            </div>
        </div>
    `;
    document.body.appendChild(modal);
    ...
    document.getElementById('amIntentModalClose').onclick = close;
    document.getElementById('amIntentModalCancel').onclick = close;
    document.getElementById('amIntentModalSubmit').onclick = async () => {
        const selfIntent = document.getElementById('amIntentSelf').value.trim();
        const familyIntent = document.getElementById('amIntentFamily').value.trim();
        close();

        const modal2 = document.getElementById('amAssessmentModal');
        const content = document.getElementById('amAssessmentContent');
        if (modal2 && content) {
            content.innerHTML = `
...
// === 支援計画生成コア ===
// opts: { selfIntent, familyIntent, medicationNote, concernsNote, monitoringText }
async function amGeneratePlanCore(assessment, opts = {}) {
        ...
        // スタッフが聞き取った意向（自由記述）はそのまま計画書に反映する
        const hasIntent = !!(opts.selfIntent || opts.familyIntent);
        const intentBlock = hasIntent ? `
【利用児及び家族の生活に対する意向（スタッフ聞き取り・原文）】
本人: ${opts.selfIntent || '（聞き取りなし）'}
家族: ${opts.familyIntent || '（聞き取りなし）'}
※この聞き取り内容を計画全体（支援方針・目標・支援内容）に反映してください。
` : '';

        const monitoringBlock = opts.monitoringText ? `
【直近のモニタリング（成長の振り返り）】
${opts.monitoringText}
※前回計画からの成長と残された課題を踏まえて、目標・支援内容を更新してください。
` : '';

        const updateBlock = (opts.medicationNote || opts.concernsNote) ? `
【アセスメント更新事項（スタッフ聞き取り）】
${opts.medicationNote ? `服薬・体調の変化: ${opts.medicationNote}` : ''}
...
${intentBlock}${monitoringBlock}${updateBlock}
...
JSONのみを出力してください。`;
        ...
        return { planFileName, supportPlanHTML };
}
...
// === 成長振り返り生成コア ===
async function amGenerateReviewCore(assessment) {
        const supportPlans = await dataAdapter.getSupportPlans();
        const dailyReports = await dataAdapter.getDailyReports();
        const reviews = await dataAdapter.getReviews();
...
`;
        ...
        return { reviewFileName, reviewHTML };
}
...
// ============================================================
// === 計画書更新ウィザード ===
// 「成長の振り返り（モニタリング）→ アセスメント更新（意向・服薬・課題の聞き取り）→ 計画書自動作成」
// の流れを1つの画面で順に進められるようにする
// ============================================================

let amRenewalState = null;
...
function amRenewalStepHeader(step) {
    const steps = ['① 成長の振り返り', '② アセスメント更新', '③ 計画書自動作成'];
    let html = `<h2 style="color:#2e7d32; margin-bottom:8px;">計画書更新：${amEscapeHtml(amRenewalState.childName)}さん</h2>`;
    html += '<div style="display:flex; gap:8px; margin:0 0 20px; flex-wrap:wrap;">';
    steps.forEach((s, i) => {
        const n = i + 1;
        const style = n === step
            ? 'background:#2e7d32; color:white; font-weight:bold;'
            : (n < step ? 'background:#c8e6c9; color:#2e7d32;' : 'background:#eee; color:#999;');
        html += `<span style="padding:6px 14px; border-radius:16px; font-size:0.85rem; ${style}">${s}</span>`;
    });
    html += '</div>';
    return html;
}

window.amRenewalRenderStep = async function(step) {
    const modal = document.getElementById('amAssessmentModal');
    const content = document.getElementById('amAssessmentContent');
    if (!modal || !content || !amRenewalState) return;

    if (step === 1) {
        // 既存の振り返りの有無を確認
        const reviews = await dataAdapter.getReviews();
        const childReviews = Object.entries(reviews)
            .filter(([_, r]) => r.childName === amRenewalState.childName)
            .sort((a, b) => new Date(b[1].createdAt) - new Date(a[1].createdAt));
        const latestInfo = childReviews.length > 0
            ? `直近の振り返り: ${new Date(childReviews[0][1].createdAt).toLocaleDateString('ja-JP')} 作成`
            : '振り返りはまだ作成されていません';

        content.innerHTML = `
            ${amRenewalStepHeader(1)}
            <p style="line-height:1.8; color:#444;">
                まず、これまでの記録をもとに<strong>成長の振り返り（モニタリング）</strong>を作成します。<br>
                <span style="font-size:0.9rem; color:#777;">${latestInfo}</span>
            </p>
            <div style="display:flex; gap:10px; margin-top:16px; flex-wrap:wrap;">
                <button class="am-btn am-btn-quaternary" onclick="amRenewalCreateReview()">振り返りを作成する</button>
                ${childReviews.length > 0 ? '<button class="am-btn" style="background:#eee;" onclick="amRenewalUseLatestReview()">直近の振り返りを使って次へ</button>' : ''}
                <button class="am-btn" style="background:#eee;" onclick="amRenewalRenderStep(2)">振り返りなしで次へ</button>
            </div>
        `;
        modal.classList.add('active');
    } else if (step === 2) {
        content.innerHTML = `
            ${amRenewalStepHeader(2)}
            <p style="line-height:1.8; color:#444; margin-bottom:12px;">
                保護者・本人から聞き取った内容を入力してください。<br>
                <strong>意向はそのまま計画書の「利用児及び家族の生活に対する意向」欄に記載</strong>されます。
                入力内容はアセスメントの更新版として保存されます。
            </p>
            <label style="display:block; font-weight:bold; margin-bottom:4px;">本人の生活に対する意向</label>
            <textarea id="amRenewalSelfIntent" rows="3" style="width:100%; padding:8px; border:1px solid #ddd; border-radius:8px; box-sizing:border-box;" placeholder="例：サッカーがうまくなりたい。試合に出たい。">${amEscapeHtml(amRenewalState.selfIntent)}</textarea>
            <label style="display:block; font-weight:bold; margin:12px 0 4px;">家族の生活に対する意向</label>
            <textarea id="amRenewalFamilyIntent" rows="3" style="width:100%; padding:8px; border:1px solid #ddd; border-radius:8px; box-sizing:border-box;" placeholder="例：気持ちの切り替えができるようになってほしい。">${amEscapeHtml(amRenewalState.familyIntent)}</textarea>
            <label style="display:block; font-weight:bold; margin:12px 0 4px;">服薬・体調の変化（任意）</label>
            <textarea id="amRenewalMedication" rows="2" style="width:100%; padding:8px; border:1px solid #ddd; border-radius:8px; box-sizing:border-box;" placeholder="例：〇月から服薬内容が変更。">${amEscapeHtml(amRenewalState.medicationNote)}</textarea>
            <label style="display:block; font-weight:bold; margin:12px 0 4px;">新たな課題・困りごと（任意）</label>
            <textarea id="amRenewalConcerns" rows="2" style="width:100%; padding:8px; border:1px solid #ddd; border-radius:8px; box-sizing:border-box;" placeholder="例：学校で友だちとのトラブルが増えている。">${amEscapeHtml(amRenewalState.concernsNote)}</textarea>
            <div style="display:flex; gap:10px; margin-top:16px; flex-wrap:wrap;">
                <button class="am-btn" style="background:#eee;" onclick="amRenewalRenderStep(1)">戻る</button>
                <button class="am-btn am-btn-secondary" onclick="amRenewalSaveAssessmentUpdate()">アセスメントを更新して次へ</button>
            </div>
        `;
        modal.classList.add('active');
    } else if (step === 3) {
        const intentPreview = (amRenewalState.selfIntent || amRenewalState.familyIntent)
            ? `本人: ${amEscapeHtml(amRenewalState.selfIntent || '（未入力）')}<br>家族: ${amEscapeHtml(amRenewalState.familyIntent || '（未入力）')}`
            : '（未入力のため、意向欄はAIがアセスメントから推定します）';
        content.innerHTML = `
            ${amRenewalStepHeader(3)}
            <p style="line-height:1.8; color:#444;">以下の内容をもとに計画書を自動作成します。</p>
            <div style="background:#f5f5f5; border-radius:8px; padding:12px; font-size:0.9rem; line-height:1.7; margin:10px 0;">
                <strong>モニタリング:</strong> ${amRenewalState.reviewHTML ? '振り返りの内容を反映します' : '振り返りなし（アセスメントのみで作成）'}<br>
                <strong>意向（聞き取り）:</strong><br>${intentPreview}
            </div>
            <div style="display:flex; gap:10px; margin-top:16px; flex-wrap:wrap;">
                <button class="am-btn" style="background:#eee;" onclick="amRenewalRenderStep(2)">戻る</button>
                <button class="am-btn am-btn-primary" onclick="amRenewalGeneratePlan()">計画書を自動作成する</button>
            </div>
        `;
        modal.classList.add('active');
    }
...
window.amRenewalCreateReview = async function() {
    const content = document.getElementById('amAssessmentContent');
    try {
        content.innerHTML = `
            ${amRenewalStepHeader(1)}
        ...
        `;
        const result = await amGenerateReviewCore(amRenewalState.assessment);
        amRenewalState.reviewHTML = result.reviewHTML;
        content.innerHTML = `
            ${amRenewalStepHeader(1)}
            <p style="color:#2e7d32; font-weight:bold;">成長の振り返りを作成しました。内容を確認して次へ進んでください。</p>
            <div style="max-height:320px; overflow-y:auto; border:1px solid #ddd; border-radius:8px; padding:12px; margin:10px 0; background:#fafafa;">${result.reviewHTML}</div>
            <div style="display:flex; gap:10px; margin-top:12px;">
                <button class="am-btn am-btn-secondary" onclick="amRenewalRenderStep(2)">次へ（アセスメント更新）</button>
            </div>
        `;
    } catch (error) {
...
window.amRenewalUseLatestReview = async function() {
    const reviews = await dataAdapter.getReviews();
    const childReviews = Object.entries(reviews)
        .filter(([_, r]) => r.childName === amRenewalState.childName)
        .sort((a, b) => new Date(b[1].createdAt) - new Date(a[1].createdAt));
    if (childReviews.length > 0) {
        const latest = await dataAdapter.getReviewWithHtml(childReviews[0][0]);
        amRenewalState.reviewHTML = latest?.html || childReviews[0][1].html || null;
    }
    amRenewalRenderStep(2);
};

window.amRenewalSaveAssessmentUpdate = async function() {
    const selfIntent = document.getElementById('amRenewalSelfIntent').value.trim();
    const familyIntent = document.getElementById('amRenewalFamilyIntent').value.trim();
    const medicationNote = document.getElementById('amRenewalMedication').value.trim();
    const concernsNote = document.getElementById('amRenewalConcerns').value.trim();

    if (!selfIntent && !familyIntent) {
        if (!confirm('意向が未入力です。計画書の意向欄はAIが推定しますが、このまま進みますか？')) return;
    }

    amRenewalState.selfIntent = selfIntent;
    amRenewalState.familyIntent = familyIntent;
    amRenewalState.medicationNote = medicationNote;
    amRenewalState.concernsNote = concernsNote;

    // 聞き取り内容があればアセスメントの更新版として保存する
    if (selfIntent || familyIntent || medicationNote || concernsNote) {
        try {
            const base = amRenewalState.assessment;
            const dateStr = new Date().toLocaleDateString('ja-JP');
            const updatedData = Object.assign({}, base.data, {
                selfIntent: selfIntent || base.data.selfIntent || '',
                familyIntent: familyIntent || base.data.familyIntent || '',
                medicationNote: medicationNote || '',
                concernsNote: concernsNote || '',
                intentUpdatedAt: new Date().toISOString()
            });
            const updateSection = `
<div style="margin-top:20px; border:2px solid #2e7d32; border-radius:8px; padding:15px;">
    <h3 style="color:#2e7d32; margin-bottom:10px;">アセスメント更新（${dateStr} 聞き取り）</h3>
    ${selfIntent ? `<p><strong>本人の生活に対する意向：</strong>${amEscapeHtml(selfIntent)}</p>` : ''}
    ${familyIntent ? `<p><strong>家族の生活に対する意向：</strong>${amEscapeHtml(familyIntent)}</p>` : ''}
    ${medicationNote ? `<p><strong>服薬・体調の変化：</strong>${amEscapeHtml(medicationNote)}</p>` : ''}
    ${concernsNote ? `<p><strong>新たな課題・困りごと：</strong>${amEscapeHtml(concernsNote)}</p>` : ''}
</div>`;
            const updatedHtml = (base.html || '') + updateSection;
            const newFileName = `${amRenewalState.childName}_アセスメント更新_${new Date().toISOString().split('T')[0]}.html`;
            await dataAdapter.saveAssessment(newFileName, updatedHtml, updatedData);
...
window.amRenewalGeneratePlan = async function() {
    const content = document.getElementById('amAssessmentContent');
    try {
        content.innerHTML = `
            ${amRenewalStepHeader(3)}
            <div class="ai-loading">
                <img src="soccerball.png" alt="読み込み中" class="ai-loading-ball">
                <span class="ai-loading-text">AIが計画書を生成中...</span>
            </div>
        `;
        const assessment = amRenewalState.updatedAssessment || amRenewalState.assessment;
        const result = await amGeneratePlanCore(assessment, {
            selfIntent: amRenewalState.selfIntent,
            familyIntent: amRenewalState.familyIntent,
            medicationNote: amRenewalState.medicationNote,
            concernsNote: amRenewalState.concernsNote,
            monitoringText: amRenewalState.reviewHTML ? amStripHtml(amRenewalState.reviewHTML) : ''
        });
        content.innerHTML = `
            <div style="background:#e8f5e9; border:1px solid #a5d6a7; border-radius:8px; padding:12px 16px; margin-bottom:16px; color:#2e7d32; font-weight:bold;">
                計画書の更新が完了しました（振り返り → アセスメント更新 → 計画書作成）
            </div>
            ${result.supportPlanHTML}
        `;
        // 児童一覧の件数バッジを更新
        amLoadChildren();
    } catch (error) {
